...
R_EFF_DEFAULT_MM = 180.0     # effective roll radius R' [mm]
A_KEY_DECIMALS = 3           # rounding for 'a' cache keys
DEFAULT_WIDTH_MM = 100.0     # strip width b [mm] (used silently)
PHIDOT_DEFAULT = 1.0         # default strain rate [1/s] for LB stress
PRINT_TORQUE_IN_NM_TOO = True  # also show torque in N·m

# ---------------- image folders & basenames ----------------
SEARCH_DIRS = [
    Path("/Photos"),  # your Windows folder
    Path("/data"),                               # fallback copies
]
...
# =============================================================================
# Landolt–Börnstein flow stress (your LB block)
# =============================================================================
...
if __name__ == "__main__":
    ...
    print(f"a = μ * sqrt(R′/h1) = {mu:.3f} * sqrt({R_eff_mm:.1f}/{h1:.3f}) = {a:.3f}  (cache key: {a_key:.{A_KEY_DECIMALS}f})")
    print(f"f1(r={r:.2f} %, a≈{a_key:.{A_KEY_DECIMALS}f}) ≈ {f1:.6f}")
    print(f"f2(r={r:.2f} %, a≈{a_key:.{A_KEY_DECIMALS}f}) ≈ {f2:.6f}")

    # -------- Force & Torque from f1/f2 (use defaults silently) --------
    b_mm = DEFAULT_WIDTH_MM
    F_coeff_kN  = roll_force_from_f1(f1, b_mm, h1)
    T_coeff_kNm = roll_torque_from_f2(f2, b_mm, h1, R_eff_mm)

    if PRINT_TORQUE_IN_NM_TOO:
        print(f"[From f1/f2]  F ≈ {F_coeff_kN:.6f} kN   |   T ≈ {T_coeff_kNm:.6f} kN·m  ({T_coeff_kNm*1000.0:.3f} N·m)")
    else:
        print(f"[From f1/f2]  F ≈ {F_coeff_kN:.6f} kN   |   T ≈ {T_coeff_kNm:.6f} kN·m")

    # -------- Deformation (flow) stress via LB (uses default φdot, no extra prompt) --------
    phidot = PHIDOT_DEFAULT
    sigma_mg = float(sigma_LB(MATERIALS["mg"], theta, phi, phidot, strict=False))
    sigma_al = float(sigma_LB(MATERIALS["al"], theta, phi, phidot, strict=False))
    sigma_avg = float(weighted_avg_stress(sigma_mg, sigma_al))

    # Basic flow-stress force/torque (for comparison)
    h_out = float(exit_thickness_from_strain(h1, phi))
    F_basic_kN  = roll_force_basic(sigma_avg, b_mm, h1 - h_out)
    T_basic_kNm = roll_torque_basic(F_basic_kN, R_eff_mm)

    print("\n--- Deformation stress (LB) & basic estimates ---")
    print(f"σ_Mg(AZ31) = {sigma_mg:.2f} MPa | σ_Al(99.5) = {sigma_al:.2f} MPa | σ̄ = {sigma_avg:.2f} MPa")
    print(f"h_out = {h_out:.4f} mm,  Δh = {h1 - h_out:.4f} mm")
    if PRINT_TORQUE_IN_NM_TOO:
        print(f"[Basic σ̄]    F ≈ {F_basic_kN:.6f} kN   |   T ≈ {T_basic_kNm:.6f} kN·m  ({T_basic_kNm*1000.0:.3f} N·m)")
    else:
        print(f"[Basic σ̄]    F ≈ {F_basic_kN:.6f} kN   |   T ≈ {T_basic_kNm:.6f} kN·m")
    # =============================================================================
