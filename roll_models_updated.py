...
# =============================================================================
# Image Handling - Uses your actual pictures
# =============================================================================
def get_image_path(filename):
    """
    Get image path from current working directory
    Uses your actual pictures: Roll pressure function.PNG and Roll Torque function.PNG
    """
...
# =============================================================================
class LBParams:
    def __init__(self, A, m1, m2, m4, m5, m7, m8, theta_range, phi_range, phidot_range, name):
        self.A = float(A)
        self.m1 = float(m1)
        self.m2 = float(m2)
        self.m4 = float(m4)
...
def save_digitization_data(fig_name, a_value, x_points, y_points, calibrated_x, calibrated_y):
...
def _digitize_single_curve(image_path, label, reductions_grid, x_defaults=(0.0, 100.0), y_defaults=(0.0, 1.0), a_value=0.0):
    """
    ...
    if len(xs_val) >= 2:
        ...
        # Filter points within reduction range
        mask = [(x >= reductions_grid.min()) and (x <= reductions_grid.max()) for x in xs_val_sorted]
        ...
        if len(xs_val_filtered) >= 2:
            ...
            x_interp = np.linspace(float(reductions_grid.min()), float(reductions_grid.max()), 100)
            ...
            plt.plot(x_px_interp, y_px_interp, 'b-', linewidth=3, alpha=0.7, label='Interpolated curve')
...
def _ensure_curve_value(fig_name, image_path, y_defaults, a_value, r_query, cache_basename):
    """
...
def f1_value(r_percent, a_value):
    ...
    return _ensure_curve_value(
        ...
        image_path=f1_img,
        y_defaults=(0.0, 90.0),  # Based on your chart range
        a_value=a_value,
...
def f2_value(r_percent, a_value):
    ...
    return _ensure_curve_value(
        ...
        image_path=f2_img,
        y_defaults=(0.0, 0.7),   # Based on your chart range
        a_value=a_value,
        r_query=float(np.clip(r_percent, _RPTS.min(), _RPTS.max())),
