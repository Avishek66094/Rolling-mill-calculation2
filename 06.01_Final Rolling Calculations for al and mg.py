...
# =============================================================================
def calculate_geometry(h1_mm, reduction_ratio_percent, R_mm):
    """
    Returns: h2_mm, delta_h_mm, ld_mm, reduction_percent
    """
    h1_mm = float(h1_mm)
    reduction_percent = float(reduction_ratio_percent)
    R_mm = float(R_mm)

    h2_mm = float((h1_mm * (100.0 - reduction_percent)) / 100.0)
    delta_h_mm = float(h1_mm - h2_mm)

    inside = R_mm * delta_h_mm - (delta_h_mm ** 2) / 4.0
    ld_mm = float(math.sqrt(inside)) if inside > 0.0 else 0.0

    return h2_mm, delta_h_mm, ld_mm, reduction_percent


def calculate_strain_parameters(h1_mm, h2_mm, v_mps, ld_mm):
    """
    Returns: phi, phi_dot, engineering strain eps
    NOTE: phi_dot uses your original structure; if you want unit-consistent,
          convert ld_mm -> ld_m inside this function.
    """
    h1_mm = float(h1_mm)
    h2_mm = float(h2_mm)
    v_mps = float(v_mps)
    ...
    if h2_mm <= 0.0 or h1_mm <= 0.0:
        raise ValueError("h1 and h2 must be > 0.")

    phi = float(math.log(h1_mm / h2_mm))

    # IMPORTANT: ld is in mm while v is m/s (unit mismatch).
    # If you want unit-consistent strain rate: use ld_m = ld_mm/1000.0
    phi_dot = float((v_mps / ld_mm) * phi) if ld_mm > 0 else float(PHIDOT_DEFAULT)

    eps = float((h1_mm - h2_mm) / h1_mm)
    return phi, phi_dot, eps
...
# =============================================================================
# FEB analytical part (corrected): f1, f2 depend on (a, eps)
# a = μ * sqrt(R/h1), eps = (h1-h2)/h1
# =============================================================================
...
def function_three(a: float, eps: float) -> float:
    """Force function (mapped to your f1)."""
    a = float(a)
    eps = _safe_eps(eps)
...
def function_four(a: float, eps: float) -> float:
    """Torque function (mapped to your f2)."""
    a = float(a)
    eps = _safe_eps(eps)
...
def roll_force_P(Km_MPa: float, Bm_mm: float, R_mm: float, delta_h_mm: float, f1: float) -> float:
    """
    P = 1.15 * Km * Bm * sqrt(R*Δh − (Δh^2)/4) * f1
    """
    Km_MPa = float(Km_MPa)
    Bm_mm = float(Bm_mm)
    R_mm = float(R_mm)
...
# =============================================================================
# Basic comparison (optional) - kept from your earlier approach
# =============================================================================
...
def perform_rolling_calculation():
    """Main function to perform rolling calculation with user inputs (THESIS formulas)"""

    print("=== Rolling Mill Calculation (FEB analytical f1/f2 + Thesis P/T) ===")
    ...
    try:
        ...
        h1 = float(input("Initial thickness h1 [mm]: ").strip())   # initial height
        v = float(input("Rolling speed v [m/s]: ").strip())
        R = float(input("Roll radius R [mm]: ").strip())
        bm = float(input("Strip width Bm [mm]: ").strip())
    ...
    if h2 <= 0.0:
        print("Computed h2 <= 0. Check inputs (reduction too high or h1 invalid).")
        return

    # -------------------- strain parameters --------------------
    try:
        phi, phi_dot, eps = calculate_strain_parameters(h1, h2, v, ld)
    except Exception as e:
        print(f"Strain parameter error: {e}")
        return

    eps_safe = float(np.clip(eps, 1e-9, 1.0 - 1e-9))

    print("Calculated parameters:")
    print(f"  Exit thickness h2 = {h2:.6f} mm")
    print(f"  Height difference Δh = {delta_h:.6f} mm")
    print(f"  Deformation/contact length ld = {ld:.6f} mm")
    print(f"  True strain φ = {phi:.6f}")
    print(f"  Strain rate φ̇ = {phi_dot:.6f} 1/s")
    print(f"  Engineering reduction ε = {eps:.6f}")

    # -------------------- FEB coefficients (corrected) --------------------
    print("\n=== FEB analytical coefficients (corrected: a uses h1, input is ε) ===")
    try:
        a_val = a_feb(mu, R, h1)          # IMPORTANT: uses initial thickness h1
        f1 = f1_value(mu, R, h1, eps_safe)
        f2 = f2_value(mu, R, h1, eps_safe)
    ...
    # -------------------- deformation stress (Km) --------------------
    print("\n=== Calculating Deformation Stress (Km) ===")
    try:
        ...
        # Map Km to mean deformation stress (as used in your thesis equations)
        Km = float(sigma_avg)

        print(f"  σ_Mg(AZ31) = {sigma_mg:.3f} MPa")
        print(f"  σ_Al(99.5) = {sigma_al:.3f} MPa")
        print(f"  σ_average (weighted) = {sigma_avg:.3f} MPa")
        print(f"  Km (used in P/T) = {Km:.3f} MPa")
    ...
    try:
        ...
        if PRINT_TORQUE_IN_NM_TOO:
            print(f"  T = (2*Km*Bm*R*((h1^2)/h2)*f2)*1e-3 = {T_value:.6f} ({T_value*1000.0:.3f} N·m)")
        else:
            print(f"  T = (2*Km*Bm*R*((h1^2)/h2)*f2)*1e-3 = {T_value:.6f}")

    except Exception as e:
        print(f"Error in P/T calculation: {e}")
        P_value = 0.0
        T_value = 0.0

    # -------------------- basic comparison (optional) --------------------
    ...
    try:
        ...
        print(f"  F_basic = {F_basic_kN:.6f} kN")
        if PRINT_TORQUE_IN_NM_TOO:
            print(f"  T_basic = {T_basic_kNm:.6f} kN·m ({T_basic_kNm*1000.0:.3f} N·m)")
        else:
            print(f"  T_basic = {T_basic_kNm:.6f} kN·m")
    ...
    results_data = {
        "h2_mm": float(h2),
        "delta_h_mm": float(delta_h),
        "ld_mm": float(ld),

        "true_strain_phi": float(phi),
        "strain_rate_phidot": float(phi_dot),
        "engineering_reduction_eps": float(eps),

        "a_parameter": float(a_val),
        "f1": float(f1),
        "f2": float(f2),
        ...
        "F_basic_kN": float(F_basic_kN),
        "T_basic_kNm": float(T_basic_kNm),
    }
